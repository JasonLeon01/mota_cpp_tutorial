...
.. hint::  自3.0版本始，事件名称DSL的格式已经更改，目前为更符合观感的“类函数”格式。
...
可以看出，时间名DSL的格式是类函数格式的，请务必保证括号对齐，否则会出现错误，当然，在使用不需要“参数”的DSL例如墙壁时，可以省略括号。
...
一般的输入法，全半角转换的快捷键为 ``shift+space`` ，微软拼音输入法打开快捷键的方法为：
 找到任务栏 :guilabel:`中/英` 字样 -> 右键点击 :guilabel:`选择设置` -> :guilabel:`按键` -> :guilabel:`全/半角切换` 。
